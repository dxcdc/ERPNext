(function() {
    ...
    function renderOngsysDashboard(data) {
        ...
        host.innerHTML = `
            ...
            <div class="cdc-admin-map-bulk"><p class="cdc-admin-map-results" data-cdc-map-results>Exibindo ${visibleMappings.length} de ${mappings.length} mapeamentos.</p><div><button class="btn btn-sm btn-default" data-cdc-map-validate-selected>Validar selecionados</button> <button class="btn btn-sm btn-default" data-cdc-map-manual-selected>Ativar manualmente selecionados</button> <button class="btn btn-sm btn-primary" data-cdc-map-activate-selected>Ativar validados</button></div></div>
    ...
    $(document).on('click', '[data-cdc-map-manual-selected]', function() { var names = Object.keys(selectedMappings).filter(function(n) { return selectedMappings[n]; }); if (!names.length) { frappe.msgprint('Selecione ao menos um mapeamento.'); return; } frappe.prompt([{fieldname:'reason',fieldtype:'Small Text',label:'Justificativa obrigatória para o lote',reqd:1}], function(v) { frappe.call({method:'cdc_theme.api.manually_activate_ongsys_warehouse_mappings',type:'POST',args:{names:JSON.stringify(names),reason:v.reason},freeze:true,callback:function(){selectedMappings={};loadOngsysDashboard();}}); }, 'Ativação manual em lote', 'Ativar selecionados'); });
