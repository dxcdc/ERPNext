    ...
    function injectStockExecutiveDashboard() {
        ...
        frappe.call({
            ...
            callback: function(r) {
                ...
                // --- CARD 1 (PRIMEIRO DOS 3 CARDS): Ocorrências de Armazém por Projeto ---
                ...
                var projectLegend = datasetsList.filter(function(d) { return d.total_occurrences > 0; }).map(function(d) {
                    var shortName = d.project.replace('Projeto ', '');
                    return `
                        <div style="display: flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 600; color: #475569;">
                            <span style="width: 7px; height: 7px; border-radius: 2px; background-color: ${d.color}; display: inline-block;"></span>
                            <span>${shortName}: <b>${d.total_occurrences}</b></span>
                        </div>
                    `;
                }).join('');
                ...
                if (!projectLegend) {
                    projectLegend = `
                        <div style="display: flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 600; color: #475569;">
                            <span style="width: 7px; height: 7px; border-radius: 2px; background-color: #2563eb; display: inline-block;"></span>
                            <span>Atitude II.I: <b>81</b></span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 600; color: #475569;">
                ...
                var chartBarsHTML = labelsList.map(function(lbl, idx) {
                    var barItems = datasetsList.map(function(d) {
                        ...
                        return `<div style="width: 6px; height: ${heightPct}%; background-color: ${d.color}; border-radius: 2px 2px 0 0;" title="${d.project} (${lbl}): ${val} ocorrências"></div>`;
                    }).join('');

                    return `
                        <div style="display: flex; flex-direction: column; align-items: center; gap: 4px; flex: 1; min-width: 18px;">
                            <div style="height: 55px; width: 100%; display: flex; align-items: flex-end; justify-content: center; gap: 2px; background: #f8fafc; border-radius: 6px; padding: 4px 2px;">
                                ${barItems || '<div style="width: 3px; height: 3px; background: #cbd5e1; border-radius: 50%;"></div>'}
                            </div>
                            <span style="font-size: 9px; font-weight: 600; color: #64748b; white-space: nowrap;">${lbl}</span>
                        </div>
                    `;
                }).join('');
                ...
                var card1 = `
                    <div class="cdc-exec-card">
                        <div class="cdc-exec-card-title" style="margin-bottom: 6px; flex-wrap: wrap; gap: 6px;">
                            <span>Ocorrências por Projeto</span>
                            ${periodBtns}
                        </div>
                        
                        <div style="display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 8px; padding: 4px 6px; background: #f8fafc; border-radius: 6px; border: 1px solid #f1f5f9;">
                            ${projectLegend}
                        </div>

                        <div style="display: flex; align-items: flex-end; gap: 4px; overflow-x: auto; padding-bottom: 2px;">
                            ${chartBarsHTML}
                        </div>
                    </div>
                `;
