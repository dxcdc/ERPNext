  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      docker exec -i nexterp-db-1 mysql -u root -p'${var.db_password}' "${var.db_name}" -e "
        ...
        DELETE FROM tabWorkspace WHERE name IN ('CDC Estoque', 'CDC Usuários', 'CDC Grupos', 'CDC Itens', 'CDC Integrações', 'CDC Pendências', 'CDC Monitoramento', 'CDC Testes', 'CDC Admin');
        ...
        UPDATE tabWorkspace SET is_hidden = 0 WHERE name IN ('CDC Estoque', 'CDC Usuários', 'CDC Grupos', 'CDC Itens', 'CDC Integrações', 'CDC Pendências', 'CDC Monitoramento', 'CDC Testes', 'CDC Admin');
